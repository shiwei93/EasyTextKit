...
class AttributedStringStyleTests: XCTestCase {
    ...
    func testHyphenation() {
        let hyphenation: Hyphenation = .default
        let style = Style().hyphenation(hyphenation)
        assert(style: style, query: { CGFloat($0.hyphenationFactor) }, float: CGFloat(hyphenation.rawValue), accuracy: 0.1)
    }
